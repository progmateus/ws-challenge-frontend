"use client";
import { DialogHTMLAttributes, useEffect } from "react";
import styles from "./styles.module.css";
import { Input } from "@/components/form/Input/Input";
import { Button } from "@/components/button/Button";
import { useForm } from "react-hook-form";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { CreateBrand } from "@/services/brandService";
import { useBrands } from "@/contexts/BrandsContext";
import { useDialogs } from "@/contexts/DialogsContext";

type IProps = DialogHTMLAttributes<HTMLDialogElement> & {
  isOpen: boolean;
  handleClose: () => void;
}

const signUpSchema = z.object({
  nome: z.string().min(3, "Min 3 caracteres").max(80, "Max 80 caracteres")
});

type CreateBrandProps = z.infer<typeof signUpSchema>

export const BrandDialog = ({ isOpen, handleClose }: IProps) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm<CreateBrandProps>({
    resolver: zodResolver(signUpSchema)
  });
  const { dialogBrandIsOpen, handleSetDialogBrandIsOpen } = useDialogs()

  useEffect(() => {
    if (dialogBrandIsOpen) {
      reset()
    }
  }, [dialogBrandIsOpen])

  const { brands, handleSetBrands } = useBrands();

  const handleCreate = (data: CreateBrandProps) => {
    CreateBrand(data).then((res) => {
      const { data } = res;
      handleSetBrands([...brands, data]);
      handleSetDialogBrandIsOpen(false)
    })
  }

  if (!isOpen) {
    return null;
  }

  return (
    <div className={styles.background}>
      <form onSubmit={handleSubmit(handleCreate)} className={styles.container} >
        <div className={styles.header}>
          <h2> Cadastrar Marca</h2>
          <span className={styles.pointer} onClick={handleClose}>
            <svg xmlns="http://www.w3.org/2000/svg" width="32px" height="32px" viewBox="0 0 24 24"><path fill="#212121" d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12z" /></svg>
          </span>
        </div>
        <div className={styles.body}>
          <div className={styles.containerInput}>
            <Input label="Nome" errorMessage={errors.nome?.message} {...register("nome")} />
          </div>
        </div>


        <div className={styles.dialogActions}>
          <Button title="CADASTRAR" type="submit" />
        </div>
      </form>
    </div>
  );
}
